\documentclass[xcolor=dvipsnames]{beamer}
\usepackage[utf8]{inputenc}
\usepackage{comment}
\usepackage{pgfplots}
\usepackage{pgfpages}
\usepackage{times,url}
\usepackage{environ}
\usepackage{booktabs}
\usepackage{multirow}
\usepackage[ruled,vlined]{algorithm2e}
\usepackage[dvipsnames]{xcolor}
\usepackage{dsfont}
\pgfplotsset{compat=newest}
\usetikzlibrary{pgfplots.groupplots}
\usetikzlibrary{shapes,arrows,shadows,shadows.blur,positioning,calc,arrows.meta,automata}
\usetikzlibrary{matrix,calc,decorations.pathreplacing}
\usetikzlibrary[bending]
\usepgfplotslibrary{fillbetween}
\usetheme{Frankfurt}
\usecolortheme{default}
\setbeamercolor*{structure}{bg=white,fg=black}
\setbeamertemplate{enumerate items}[default]
\makeatletter
\newcommand{\nonumsec}[1]{%
\section*{#1}%
\addtocontents{toc}{\protect\beamer@sectionintoc{\the\c@section}{#1}{\the\c@page}{\the\c@part}%
        {0}}%
}
\makeatother

\setbeamertemplate{headline}
{%
  \begin{beamercolorbox}[colsep=1.5pt]{upper separation line head}
  \end{beamercolorbox}
  \begin{beamercolorbox}{section in head/foot}
    \vskip2pt\insertnavigation{\paperwidth}\vskip2pt
  \end{beamercolorbox}%
  \begin{beamercolorbox}[colsep=1.5pt]{lower separation line head}
  \end{beamercolorbox}
}

\addtobeamertemplate{navigation symbols}{}{%
    \usebeamerfont{footline}%
    \usebeamercolor[fg]{footline}%
    \hspace{1em}%
    \insertframenumber/\inserttotalframenumber
}
\newcommand*{\xMin}{-4}%
\newcommand*{\xMax}{4}%
\newcommand*{\yMin}{-4}%
\newcommand*{\yMax}{4}%
%--------------------------------------------------------------------------------------------------
%This block of code defines the information to appear in the
%Title page
\title[] %Learning Discriminative Representations to Interpret Image Recognition Models
{Learning Discriminative Representations to Interpret Image Recognition Models}
\subtitle{Thèse de Doctorat}
\author[Felipe Torres Figueroa] % (optional)
{\small Felipe Torres Figueroa}
\institute[ECM] % (optional)
{\footnotesize
  \'Ecole Centrale de Marseille
  \and
  QARMA - Laboratoire d'Informatique et de Syst\`emes (LIS)
}
\date[2024] % (optional)
{\small Marseille, September $23^{rd}$ 2024}
\logo{%
  \makebox[0.95\paperwidth]{%
    \includegraphics[height=1cm,keepaspectratio]{logo/logo-lis-2024-eps-converted-to.pdf}%    
    \hfill%
    \includegraphics[height=1cm,keepaspectratio]{logo/ecm_logo.png}%
  }%
}
%End of title page configuration block
%--------------------------------------------------------------------------------------------------
% Title -V2
%--------------------------------------------------------------------------------------------------
%The next block of commands puts the table of contents at the 
%beginning of each section and highlights the current section:
\AtBeginSection
{
  \begin{frame}
    \frametitle{Table of Contents}
    \tableofcontents[currentsection]
  \end{frame}
}
\input{tex/supp/plots}
\input{tex/supp/abbrev}
\input{tex/supp/defn}
%--------------------------------------------------------------------------------------------------
\begin{document}
	%The next statement creates the title page.
	%{
	%\setbeamertemplate{headline}{}%
	%\begin{frame}
	%	\maketitle
	%\end{frame}
	%}
	%\logo{}
  %---------------------------------------------------------
  % Title V2
  \begin{frame}[plain]
    \maketitle
    \vspace{-20pt}
    {\footnotesize
    %{\centering\itshape Jury Members\par}
    President: Fr\'ed\'eric Jurie
    \begin{tabular}[t]{@{}l@{\hspace{3pt}}p{.32\textwidth}@{}}
    Reviewers: & Fr\'ed\'eric Jurie & PR - Univ. Caen\\
    & Giorgos Tolias &Associate Professor - CTU Prague\\
    Examiners:& Fr\'ed\'eric Precioso & PR - Univ. Côte d'Azur\\
    & Diane Larlus & Principal Research Scientist - Naver Labs.
    \end{tabular}%
    %\scriptsize
    \begin{tabular}[t]{@{}l@{\hspace{3pt}}p{.3\textwidth}@{}}
    Supervisors: & St\'ephane Ayache & Aix Marseille Univ\\
     & Ronan Sicre & Centrale Marseille\\
     & Yannis Avrithis & IARAI
    \end{tabular}
    }%
    \end{frame}
    \logo{}
	%---------------------------------------------------------
	%This block of code is for the table of contents after
	%the title page
	\begin{frame}
		\frametitle{Table of Contents}
		\tableofcontents
	\end{frame}
	\input{tex/intro/main}
	\input{tex/rel/main}
	\input{tex/opticam/main}
	\input{tex/castream/main}
	\input{tex/gradient/main}
  \input{tex/conclusion}
  %\input{tex/futurework}
	\input{tex/references}
	%-----------------------------------------------------------------------------------------------
\end{document}